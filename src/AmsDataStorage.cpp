...
bool AmsDataStorage::update(AmsData* data) {
    ...
    if(debugger->isActive(RemoteDebug::VERBOSE)) debugger->printf("(AmsDataStorage) Time is: %ld\n", (int64_t) now);    //EHorvat NES-MEP %lld to %ld
    if(tz == NULL) {
    ...
    if(now < BUILD_EPOCH) {
        if(data->getMeterTimestamp() > BUILD_EPOCH) {
            ...
            if(debugger->isActive(RemoteDebug::DEBUG)) {
                debugger->printf("(AmsDataStorage) Using meter timestamp, which is: %ld\n", (int64_t) now);  //EHorvat NES-MEP %lld to %ld
            }
        } else if(data->getPackageTimestamp() > BUILD_EPOCH) {
            now = data->getPackageTimestamp();
            if(debugger->isActive(RemoteDebug::DEBUG)) {
                debugger->printf("(AmsDataStorage) Using package timestamp, which is: %ld\n", (int64_t) now);  //EHorvat NES-MEP %lld to %ld
            }
        }
    }
    if(now < BUILD_EPOCH) {
        if(debugger->isActive(RemoteDebug::VERBOSE)) {
            debugger->printf("(AmsDataStorage) Invalid time: %ld\n", (int64_t) now);  //EHorvat NES-MEP %lld to %ld
        }
        return false;
    }
    ...
    if(day.lastMeterReadTime > now) {
        ...
        if(debugger->isActive(RemoteDebug::DEBUG)) {
            debugger->printf("(AmsDataStorage) Last day update: %ld\n", (int64_t) day.lastMeterReadTime);  //EHorvat NES-MEP %lld to %ld
        }
        tmElements_t last;
        ...
        if(day.activeImport == 0 || now - day.lastMeterReadTime > 86400) {
            ...
            while(day.lastMeterReadTime < stopAt) {
                ...
                if(debugger->isActive(RemoteDebug::INFO)) {
                    debugger->printf("(AmsDataStorage) Estimated usage for hour %u: %.1f - %.1f (%ld)\n", last.Hour, imp, exp, (int64_t) cur);     //EHorvat NES-MEP %lld to %ld
                }
    ...
    if(ltz.Hour == 0 && !isMonthHappy()) {
        if(month.activeImport == 0 || now - month.lastMeterReadTime > 2678400) {
            ...
            if(debugger->isActive(RemoteDebug::DEBUG)) {
                debugger->printf("(AmsDataStorage) Last month read after resetting to midnight: %ld\n", (int64_t) month.lastMeterReadTime);   //EHorvat NES-MEP %lld to %ld
            }
            ...
            while(month.lastMeterReadTime < stopAt) {
                ...
                if(debugger->isActive(RemoteDebug::INFO)) {
                    debugger->printf("(AmsDataStorage) Estimated usage for day %u: %.1f - %.1f (%ld)\n", last.Day, imp, exp, (int64_t) cur);    //EHorvat NES-MEP %lld to %ld
                }
...
bool AmsDataStorage::isDayHappy() {
    ...
    if(now < day.lastMeterReadTime) {
        if(debugger->isActive(RemoteDebug::VERBOSE)) debugger->printf("(AmsDataStorage) Day %ld < %ld\n", (int64_t) now, (int64_t) day.lastMeterReadTime); //EHorvat NES-MEP %lld to %ld
        return false;
    }
    if(now-day.lastMeterReadTime > 3600) {
        if(debugger->isActive(RemoteDebug::VERBOSE)) debugger->printf("(AmsDataStorage) Day %lld - %ld > 3600\n", (int64_t) now, (int64_t) day.lastMeterReadTime); //EHorvat NES-MEP %lld to %ld
        return false;
    }
...
bool AmsDataStorage::isMonthHappy() {
    ...
    if(now < month.lastMeterReadTime) {
        if(debugger->isActive(RemoteDebug::VERBOSE)) debugger->printf("(AmsDataStorage) Month %ld < %ld\n", (int64_t) now, (int64_t) month.lastMeterReadTime); //EHorvat NES-MEP %lld to %ld
        return false;
    }
    if(now-month.lastMeterReadTime > 86400) {
        if(debugger->isActive(RemoteDebug::VERBOSE)) debugger->printf("(AmsDataStorage) Month %ld - %ld > 3600\n", (int64_t) now, (int64_t) month.lastMeterReadTime); //EHorvat NES-MEP %lld to %ld
        return false;
    }
    breakTime(tz->toLocal(now), tm);
    breakTime(tz->toLocal(month.lastMeterReadTime), last);
