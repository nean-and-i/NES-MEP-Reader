...
#include "NES-MEP.h"      //   EHorvat NES-MEP   ********
#include "NES-MEP-Tools.h"//   EHorvat NES-MEP   ********
...
void configFileParse() {
	...
	while((size = file.readBytesUntil('\n', buf, 1024)) > 0) {
		if(strncmp(buf, "boardType ", 10) == 0) {
			...
			if(!lMeter) { config.getMeterConfig(meter); lMeter = true; };
			fromHex(meter.encryptionKey, String(buf+19), 21);  //EHorvat (21 was 16)
	...
	if (Debug.isActive(RemoteDebug::INFO)) debugI("Saving configuration now...");
	Serial.flush();
...
void WiFi_connect() {
	...
	if (WiFi.status() != WL_CONNECTED) {
		if(WiFi.getMode() != WIFI_OFF) {
			if(wifiReconnectCount > 4320) {  		// EHorvat 4320 --> restart 3 days interval (original number was 3)
				ESP.restart();
				return;
			}
...
void SerialEvent2() {
	while(Serial2.available() && InputBufferLength < MaxMEPReplyLength) {
		...
		if (Debug.isActive(RemoteDebug::VERBOSE)) debugD("++ Received character code: %i",InputBuffer[InputBufferLength]);    //EHorvat new
		InputBufferLength++;
...
void setup() {
	Serial.begin(115200);
	Serial2.begin(9600, SERIAL_8N1); //EHorvat NES-MEP Test (was Serial2.begin(9600)) //new... SERIAL_8N1
...
// next line moved to Serial Event2 because of ESP chrases
//	for(int i = 0; i < 20; i++)	mep_key[i] = meterConfig.encryptionKey[i];  //EHorvat now just copy hex key from cofig to mep_key

//	RS3232Enable(true);  	// EHorvat disabled this function
...
bool readHanPort() {
	if(Debug.isActive(RemoteDebug::INFO)) debugI("***** readHanPort   ******"); //  EHorvat NES-MEP
/* ++++++++++  EHorvat NES-MEP
...
void loop() {
	...
	if(gpioConfig.apPin != 0xFF) {
		if (digitalRead(gpioConfig.apPin) == LOW) {
			...
			if ((now - buttonTimer > longPressTime) && (longPressActive == false)) {
				...
				Serial.printf("AP Button was long pressed, ---> Swap wifimode \n");				//EHorvat ... always show this 
				debugD("AP Button was long pressed, ---> Swap wifimode");  						//EHorvat NES-MEP  ...just to be informed...
				swapWifiMode();
			}
		} else {
			if (buttonActive == true) {
				if (longPressActive == true) {
					...
					// Single press action
					Serial.printf("AP Button was short pressed, ---> Swap wifimode \n");		//EHorvat ... always show this 
					debugD("AP Button was clicked, no action configured");
				}
				buttonActive = false;
			}
		}
	...
	if (mep_data_ready_to_send) {   		//EHorvat NES-MEP
...
//  ++++++++++++   EHorvat NES-MEP ++++++++++
 	byte j;
  	...
  	if(millis()- mep_LastSentMillis > 5000) {    // NES-MEP   ..... send data at a 5000ms rate
    	mep_LastSentMillis = millis();		mep_data_ready = mep_alivecounter > mep_alivecounter_last;   // mep_alivecounter is increased in NESMEP.cpp line 543
...
//      		RS3232Enable(true);		
      		...
      		delay(500);   // EHorvat was 1000
      		SerialEvent2();
			if(Debug.isActive(RemoteDebug::VERBOSE)) debugD("RS3232 reset 2/2. Dropping buffer with this contents:");
...
//        		Serial.printf("Error: Input buffer overflow. InputBufferLength: %i Dropping buffer with this contents:\r\n",InputBufferLength);	//EHorvat disabled
				if(Debug.isActive(RemoteDebug::VERBOSE)) debugD("Error: Input buffer overflow. InputBufferLength: %i Dropping buffer with this contents:",InputBufferLength);  //EHorvat new
        		dumpByteArray(InputBuffer,InputBufferLength);       
        		InputBufferLength = 0;
      		}
...
//          			Serial.printf("Error: InputBufferLength: %i, got this reply from the meter, but did not expect one:\r\n",InputBufferLength);	//EHorvat disabled
					if(Debug.isActive(RemoteDebug::VERBOSE)) debugD("Error: InputBufferLength: %i, got this reply from the meter, but did not expect one:",InputBufferLength); //EHorvat new
          			dumpByteArray(InputBuffer,InputBufferLength);       
          			InputBufferLength = 0;
        		} 	
				else {
...
//          			Serial.printf("Saved request response at index %i\r\n",MEPQueueSendIndex);	//EHorvat disabled
					if(Debug.isActive(RemoteDebug::VERBOSE)) debugD("Saved request response at index %i",MEPQueueSendIndex); //EHorvat new
 ...
 //   			Serial.printf("Error: InputBufferLength: %i, got this garbage from the meter, but did not expect a package:\r\n",InputBufferLength);  //EHorvat disabled
				if(Debug.isActive(RemoteDebug::VERBOSE)) debugD("Error: InputBufferLength: %i, got this garbage from the meter, but did not expect a package:",InputBufferLength);   //EHorvat new
        		dumpByteArray(InputBuffer,InputBufferLength);       
        		InputBufferLength = 0;
      		}
    	}
...
//       		Serial.printf("Giving up on reqest at index %i - too may retries\r\n");   //EHorvat disabled
				if(Debug.isActive(RemoteDebug::VERBOSE)) debugD("Giving up on reqest at index %i - too may retries");    //EHorvat new
        		IncreaseMEPQueueIndex(&MEPQueueSendIndex);
      		}
      		else {
...
//    			Serial.printf("Transmitting request at index %i (attempt %i)\r\n",MEPQueueSendIndex,MEPQueue[MEPQueueSendIndex].SendAttempts);//EHorvat disabled
				if(Debug.isActive(RemoteDebug::VERBOSE)) debugD("Transmitting request at index %i (attempt %i)",MEPQueueSendIndex,MEPQueue[MEPQueueSendIndex].SendAttempts);    //EHorvat new
				if(Debug.isActive(RemoteDebug::VERBOSE)) debugD("Transmitting request at index %i (Request Length %i)",MEPQueueSendIndex,MEPQueue[MEPQueueSendIndex].RequestLength);    //EHorvat new
        		for(unsigned long i = 0; i < MEPQueue[MEPQueueSendIndex].RequestLength; i++) {												//EHorvat new
					if(Debug.isActive(RemoteDebug::VERBOSE)) debugD("Requests bytes: %i  %X",MEPQueue[MEPQueueSendIndex].Request[i],MEPQueue[MEPQueueSendIndex].Request[i]);    		//EHorvat new
        		} 
      		...
      		if(millis()-LastSentMillis > 5000) {     //EHorvat Nes-MEP was 2000
 				ds.update5Minute(ConsumptionData.BT28_Fwd_W - ConsumptionData.BT28_Rev_W); //EHorvat new for 5 Minute plot send Import - Export power
...
//				queueRequest("300034",mep_key,MEPQueue,&MEPQueueNextIndex,None); // BT52: UTC Clock  EHorvat added 
				...
				if(Debug.isActive(RemoteDebug::INFO)) debugI("---- mep_key ByteArray (hex) = %X %X %X %X %X %X %X %X %X %X %X %X %X %X %X %X", 
																...
																mep_keyByteArray[10],mep_keyByteArray[11],mep_keyByteArray[12],mep_keyByteArray[13],mep_keyByteArray[14],
																mep_keyByteArray[15]);
        	    if(Debug.isActive(RemoteDebug::INFO)) debugI("------ Read MEP Port ------ %d", ConsumptionData.BT28_Fwd_W); //  EHorvat NES-MEP
